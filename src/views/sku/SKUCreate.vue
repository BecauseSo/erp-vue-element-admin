        ...
        <el-row class="centen-cla">
            <el-col class="layout-cla">
                <el-row>
                    ...
                    <el-col>
                        <el-form ref="baseinfo" class="demo-ruleForm" label-width="130px" :model="modifyModel" :inline="true" :rules="baserules">
                           <el-form-item label="SPU编码">
                            ...
                            <el-form-item label="是否原创" prop="is_fake">
                            ...
                            <el-form-item label="货源地址" prop="product_link">
                                <el-input v-model="modifyModel.product_link"></el-input>
                            </el-form-item>
                            <el-form-item label="对标/视频链接" prop="video_link">
                            ...
                            <el-form-item label="产品图片">
                                <el-col>
                                    ...
                                    <input type="file" @change="displayImage" ref="imageInput" style="width:185px;">
                                </el-col>
                            </el-form-item>
                        </el-form>
                    </el-col>
            ...
            <el-col class="footer-but">
                ...
                <el-button type="primary" @click="addSPUConfirmBtnAction('modifyModel') " v-if="isCreate">添 加</el-button>
                <el-button type="primary" @click="modifySPUConfirmBtnAction() " v-else>修 改</el-button>
            </el-col>
        </el-row>
    </el-container>
    </el-main>
...
export default {
    ...
    data(){
        return{
            ...
            baserules:{
                ...
                product_link:[
                    { required: true, message: '请输入货源', trigger: 'blur' }
                ],
                video_link:[
    ...
    methods:{
        ...
        skumodifyComplete(item){
            ...
            let _this = this;
            this.http.post('/sku/apply/user/modify',params)
                    .then(result=>{
                        _this.$refs.createSKUView.close();
        ...
        uploadSPU(){
            ...
            _this.http.post('/spu/apply/add',params)
                    .then(result=>{
                        _this.$message({type:'success',message:"添加成功"});
                        _this.$router.push("/sku/mysku");
                    }).catch(error=>{
        ...
        modifySPUConfirmBtnAction(){
            if (this.modifyModel.SPU_code == this.originModel.SPU_code &&
                ...
                this.modifyModel.voltage == this.originModel.voltage && this.imageFile == null){
                this.$router.push('/sku/mysku');
                return;
            }
            if (this.imageFile == null){
                this.updateSPU();
            ...
            this.http.uploadFile('/sku/product/image/upload',this.imageFile)
                .then(result=>{
                    ...
                    let url = result.data.url;
                    this.modifyModel.product_image = url;
                    console.log('upload image sccess');
                    this.updateSPU();
                }).catch(error=>{
                    console.log(error);
                    this.$message({type:'error',message:"上传图片失败"});
        ...
        updateSPU(){
            ...
            this.http.post('/spu/apply/user/modify',params)
                    .then(result=>{
                        _this.$message({type:'success',message:"提交申请成功"});
                        this.$router.push('/sku/mysku');
                    }).catch(error=>{
